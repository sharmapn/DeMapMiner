...
SELECT id,sendername, emailaddress, senderFirstName, senderLastName, -- not sure why i put in front DISTINCT (sendername) AS MATCHING,
	NULL AS 'DUMMY',
	levenshtein_distance('Guido van Rossum',sendername) AS levenshtein_distance,'FULL SENDERNAME MATCH' AS 'match'
...
SELECT id,sendername, emailaddress, senderFirstName, senderLastName,
	levenshtein_distance('guido',senderfirstname) AS 'firstnamematch', levenshtein_distance('rossum',senderlastname) AS 'lastnamematch', 'COMBINED FIRST&LASTNAME MATCH' AS 'match'
FROM distinctsenders
...
SELECT id,sendername, emailaddress, senderFirstName, senderLastName, 
	NULL AS 'DUMMY',
	levenshtein_distance('guido rossum', CONCAT(senderfirstname,' ',senderLastName)) AS 'COMBINEDFIRSTLASTNAMEMATCH', 'EMAIL VS COMB F&L NAME MATCH' AS 'match'
FROM distinctsenders
...
SELECT id,sendername, emailaddress, senderFirstName, senderLastName,
	levenshtein_distance('gvrossum', CONCAT(SUBSTRING(senderfirstname,0,1),senderLastName)) AS 'emailVs1stLetterFirstNameRestLName',
	levenshtein_distance('gvrossum', CONCAT(senderfirstname, SUBSTRING(senderLastname,0,1))) AS 'emailVsFirstNameFirstLetterLname', 
...
SELECT id,sendername, emailaddress, senderFirstName, senderLastName, 
	NULL AS 'DUMMY',
	levenshtein_distance('guido@',emailaddress) AS levenshtein_distance,'EMAIL MATCH' AS 'match'
FROM distinctsenders
HAVING levenshtein_distance <2 -- order by t asc limit 15

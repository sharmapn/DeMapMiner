...
DROP TABLE finalsocialrolesbydmsubphases;
...
SELECT * 					-- clusterbysenderfullname,
	FROM (-- social discussion metrics
		SELECT author AS m, peptype AS peptypea, folder as foldera,
	...
	ON tbA.m = tbC.n
	AND tbA.peptypea = tbC.peptype2020
...
SELECT m, 
	...
	FROM finalsocialrolesbydmsubphases -- _bdfld -- chose from either for y
...
--	WHERE peptypea = 'Informational'  -- 'Process' -- 'Standards Track' -- Informational
   GROUP BY  m, DMsubphase -- peptypea, 
	...
	ORDER BY pepinvolvement desc,m, peptypea desc;

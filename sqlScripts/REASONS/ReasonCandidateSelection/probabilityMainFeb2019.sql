...
select * from trainingdata where pep = 234  -- order by messageid
...
proposal = 391 -- and messsageTypeIsReasonMessage =1; 
...
-- Paragraph level
SELECT @curRank := @curRank + 1 AS rank, messageTypeIsReasonMessage as RM,messageType as MT, proposal,messageid,dateValue, paragraph,termsMatched, 
...
+reasonLabelFoundUsingTripleExtractionProbabilityScore+messageContainsSpecialTermProbabilityScore+prevParagraphSpecialTermProbabilityScore-negationTermPenalty) as TotalProbability, 
label, authorRole, location,
paragraphHintProbablity,paragraphLocationHintProbability,nearbyParagraphsProbabilityScore,messageTypeIsReasonMessageProbabilityScore,messageLocationProbabilityScore,
messageSubjectHintProbablityScore,negationTermPenalty,dateDiffProbability,authorRoleProbability,messageTypeIsReasonMessageProbabilityScore,paragraphLocationInMessageProbabilityScore,
sameMsgSubAsStateTripleProbabilityScore, reasonLabelFoundUsingTripleExtractionProbabilityScore,messageContainsSpecialTermProbabilityScore
from autoextractedreasoncandidateparagraphs, (SELECT @curRank := 0) r  
where location = 'paragraph' and -- sentence like '%unprecedented%' and
proposal = 350  -- and messageid = 332054 -- and messsageTypeIsReasonMessage =1; 
order by label asc, TotalProbability desc, dateValue desc; 
-- 332054
-- and messageid = 204293
-- Message Level

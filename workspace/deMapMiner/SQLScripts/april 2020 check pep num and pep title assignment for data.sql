...
-- april 2020..note that A (email analysis chapter) has been redone 
-- to make the dates equivalent to the boundary spanners paper --- so both chapters have tyhe same dateset dates = 10th March 2017
...
-- 75,113
SELECT COUNT(DISTINCT messageid) FROM allmessages WHERE DATE2 <= '2017-03-10' 
-- 1366145
...
--first case only pep numbers in message body or subject = current total minus previous total 
-- old data scheme total
SELECT COUNT(DISTINCT messageid) FROM allmessages  
WHERE originalPEPNumber <> -1

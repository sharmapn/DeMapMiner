...
//september 2020...SIG have their own DM process - https://lwn.net/Articles/580399/
// "The developers working on packaging (a group known as "distutils-sig") no longer had to get their decisions rubber-stamped by the python-dev list; it is now the decision-making body in this area."
// So, all substates captured in that list shpwl now be in their own region. Thus they have a SIG prefix in their labels
...
//	static String startState = "draft", endState = "accepted";
	...
	static String removeSubstatesList[] = {"uncontroversial","check_in_pep","concerns_addressed","renamed","idea_postponed","co_bdfl_delegate_accepted_pep","implemented","small_edits_left",			
			"acceptance_stalled","pre-pep","bdfl final pep","implemented","checking_community_consensus","idea rejected","renamed","reviewed", "decided_bdfl_consensus", "decided bdfl consensus",
			"SIG_renamed", "sig renamed", "SIG_reviewed","sig reviewed", "SIG implemented", "sig implemented", "sig_vote", "sig vote",
			"bdfl_proposes","bdfl proposes","BDFL proposes","BDFL_proposes", "sig_bdfl_proposes", "SIG_BDFL_proposes", "sig bdfl proposes", "SIG BDFL proposes",
			"BDFL_final_pep","BDFL final pep","bdfl_final_pep","bdfl final pep", "sig_bdfl_final_pep","SIG_BDFL_final_pep","sig bdfl final pep","SIG BDFL final pep"}; //,
			//"complete_rewrite", "rewriting_needed","complete_rewrite_second_author"};
	//sept 2020		
	...
	public static void main(String[] args) {	
		...
		//ArrayList<pair> renameSpecificSubstatesForPEPs = new ArrayList<pair>();
		 ...
		 pair b91 = new pair("rewritten", "ReDraft");	renameSpecificSubstatesForPEPs.add(b91);	
		 pair b92 = new pair("revived", "ReDraft");	renameSpecificSubstatesForPEPs.add(b92);
		 pair b93 = new pair("redraft", "ReDraft");	renameSpecificSubstatesForPEPs.add(b93);	
		 //added October 2020
		 pair b10 = new pair("secondDraft", "ReDraft");	renameSpecificSubstatesForPEPs.add(b10);
		 pair b101 = new pair("thirdDraft", "ReDraft");	renameSpecificSubstatesForPEPs.add(b101);
		 pair b11 = new pair("round7", "ReDraft");	renameSpecificSubstatesForPEPs.add(b11);
		 pair b12 = new pair("round2", "ReDraft");	renameSpecificSubstatesForPEPs.add(b12);
		 pair b13 = new pair("round6", "ReDraft");	renameSpecificSubstatesForPEPs.add(b13);
		 pair b14 = new pair("round3", "ReDraft");	renameSpecificSubstatesForPEPs.add(b14);
		 pair b15 = new pair("round4", "ReDraft");	renameSpecificSubstatesForPEPs.add(b15);
		 pair b16 = new pair("round5", "ReDraft");	renameSpecificSubstatesForPEPs.add(b16);
		 //pair b13 = new pair("round6", "ReDraft");	renameSpecificSubstatesForPEPs.add(b13);
		 pair b17 = new pair("lastRound", "ReDraft");	renameSpecificSubstatesForPEPs.add(b17);
		 pair b171 = new pair("positive_feedback_again", "positive feedback");	renameSpecificSubstatesForPEPs.add(b171);
		 pair b172 = new pair("received_favourably", "positive feedback");	renameSpecificSubstatesForPEPs.add(b172);
		 pair b173 = new pair("complete_rewrite", "ReDraft");	renameSpecificSubstatesForPEPs.add(b173);
		 pair b174 = new pair("rewriting_needed", "ReDraft");	renameSpecificSubstatesForPEPs.add(b174);
		 pair b175 = new pair("complete_rewrite_second_author","ReDraft");	renameSpecificSubstatesForPEPs.add(b175);
		 
		 pair b9B = new pair("bdfl_delegate_accepted_pep", "bdfl_delegate_accepted");	renameSpecificSubstatesForPEPs.add(b9B);
		 pair b9B1 = new pair("received_favourably", "positive feedback");	renameSpecificSubstatesForPEPs.add(b9B1);
		 ...
		 pair b9D2 = new pair("ready BDFL review", "ready review");	renameSpecificSubstatesForPEPs.add(b9D2);
		...
		//oct 2020
		pair p0 = new pair("approved", "no_objections");	juggleList.add(p0);
		...
		pair p91 = new pair("accepted", "decision_overwritten");	      		juggleList.add(p91);
...
//$$						System.out.println(a1 + " __ " + b1);
				...
				//September 2020..separate SIG lists
				//took care of this by coding a prefix at the end
				//tprList = oerr.renameSIGSubstates(tprList);	
...
//xxx			fw.append("pep| author| date| label| currentSentence| reverb| clausie| ollie| repeatedLabel|ps|ns|pp|ep|np \n");
	...
	private static ArrayList <TripleProcessingResult> extractResults_PopulateTPR(Connection conn, String str,	ArrayList<TripleProcessingResult> tprList) throws SQLException {
		...
		ResultSet rs = stmt.executeQuery("SELECT pep, author,authorRole,messageSubject,numberOfPEPsMentionedInMessage, date,"
										    ...
										    // oct 2020
										    + " and label NOT LIKE '%implemented%' "  //previous version rejected
										    + " and label NOT LIKE '%renamed%' "  //previous version rejected
										    + " and label NOT LIKE '%reviewed%' "  //previous version rejected
										    + " and label NOT LIKE '%bdfl_final%' "  //previous version rejected
										    + " and label NOT LIKE '%bdfl_proposes%' "  //previous version rejected
											+ str
		...
		while (rs.next()) {
			...
			if (ignorePEPs) {
				...
				//System.out.println("Folder consider: " +folderX);
				
				String lab = "";
				lab = rs.getString("label");
				//System.out.println("Label consider: " +lab);
				...
				if (folderX!= null)
					...
					{
						lab = "SIG "+lab;
						System.out.println("Label changed: " +lab);
					}
				if (dontConsider) {
...
//					System.out.println(rs.getString("pep") + " | " + label);
					tpr.setData(pepNumber,rs.getString("author"),rs.getString("authorRole"), rs.getString("messageSubject"),rs.getInt("numberOfPEPsMentionedInMessage"),justdate ,
							...
							rs.getInt("messageID"),lab, rs.getString("subject"), 
							 rs.getString("relation"),  rs.getString("object"), rs.getString("currentSentence"), 
							rs.getString("reverb"), lab, rs.getString("ollie"),rs.getBoolean("repeatedLabel"),rs.getString("role"),
...
//				}				
	...
	private static ArrayList<TripleProcessingResult> upperCaseMainStatesInResults_removeUnderscore(ArrayList <TripleProcessingResult> tripleProcessingResult) {
		for (int x=0; x <tripleProcessingResult.size(); x++){
			...
			if (result.toLowerCase().contains("bdfl")) {
			...
			if (result.toLowerCase().equals("redraft")) {
				//result = result.replaceAll("redraft", "reDraft").trim();
				result =  "reDraft";
				tripleProcessingResult.get(x).setClausIE(result);
			}
			
			if (result.toLowerCase().startsWith("sig")) {
				//result = result.replaceAll("redraft", "reDraft").trim();
				result =  result.replaceFirst("sig","SIG");
				tripleProcessingResult.get(x).setClausIE(result);
			}

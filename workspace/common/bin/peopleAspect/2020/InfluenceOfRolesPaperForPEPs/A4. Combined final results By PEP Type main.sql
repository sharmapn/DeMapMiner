...
SELECT * 					-- clusterbysenderfullname,
	FROM (-- social discussion metrics
		SELECT author AS m, peptype, -- substring(folder,13), 
			CASE substring(folder,13) 
			...
			WHERE (author IN (SELECT author FROM top10authors) OR author = 'proposalauthor'  OR author = 'bdfl_delegate' )
				...
				-- AND pep IN (SELECT pep FROM pepdetails WHERE created < '2018-07-11' AND LENGTH(bdfl_delegate)>0 )
				-- and peptype
				-- AND peptype = 'Standards Track'
				-- if you want certain members
				AND (author LIKE '%guido%' OR author LIKE '%proposalauthor%' OR author LIKE '%nick%' OR author LIKE '%delegate%')
				GROUP BY author, peptype, DMsubphase -- folder -- , peptype -- , state -- , peptype, state
	...
	LEFT JOIN (	
		SELECT clusterbysenderfullname AS m2, COUNT(DISTINCT threadid) AS threadCNT, peptype2020,
			CASE substring(folder,13) 
					WHEN 'python-ideas' THEN 'idea_generation' WHEN 'python-dev' THEN 'idea_discussion' WHEN 'python-checkins' THEN 'idea_discussion' 
					WHEN 'python-bugs-list' THEN 'idea_discussion' WHEN 'python-committers' THEN 'idea_discussion' 
					WHEN 'python-announce-list' THEN 'idea_discussion' WHEN 'python-distutils-sig' THEN 'idea_discussion'
			...
			-- where threadlevel < 3  ...for firsta nd second level results
				-- ONLY IF U WANT THE 52 PEPS
			-- AND pepnum2020 IN (SELECT pep FROM pepdetails WHERE created < '2018-07-11' AND LENGTH(bdfl_delegate)>0 )
			-- and peptype
			-- AND peptype2020 = 'Standards Track'
			GROUP BY clusterbysenderfullname, peptype2020, DMsubphase2
			-- ORDER BY threadCNT DESC
		UNION
			SELECT authorsrole AS m2, COUNT(DISTINCT threadid) AS threadCNT, peptype2020,
			CASE substring(folder,13) 
				WHEN 'python-ideas' THEN 'idea_generation' WHEN 'python-dev' THEN 'idea_discussion' WHEN 'python-checkins' THEN 'idea_discussion' 
				WHEN 'python-bugs-list' THEN 'idea_discussion' WHEN 'python-committers' THEN 'idea_discussion' 
				WHEN 'python-announce-list' THEN 'idea_discussion' WHEN 'python-distutils-sig' THEN 'idea_discussion'
			...
			-- and peptype
			-- AND peptype2020 = 'Standards Track'
			GROUP BY authorsrole, peptype2020, DMsubphase2
		ORDER BY threadCNT DESC
		) as tbC
	-- ON tbA.clusterbysenderfullname = tbC.inReplyToUserUsingClusteredSender;
	ON tbA.m = tbC.m2 
	AND tbA.DMsubphase = tbC.DMsubphase2	
--	AND tbA.peptype = tbC.peptype2020
)  AS TBIG
GROUP BY m, peptype, DMsubphase
ORDER BY pepinvolvement desc
...
WHERE (author IN (SELECT author FROM top10authors) OR author = 'bdfl_delegate' )
	AND PEP IN (SELECT pep FROM bdflddpeps) -- (SELECT pep FROM pepdetails WHERE LENGTH (bdfl_delegatecorrected) > 0 ) --   AND created < '2017-07-12'  )
...
FROM (SELECT DISTINCT PEP, state, DATE2, peptype						
	...
	AND PEP IN (SELECT pep FROM bdflddpeps) -- (SELECT pep FROM pepdetails WHERE LENGTH (bdfl_delegatecorrected) > 0 ) 
	order by DATE2 ASC)
	AS tbA

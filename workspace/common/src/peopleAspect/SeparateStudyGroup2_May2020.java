...
//Jan 2021, i now want to add roles contributions analysis, where the bdfl's, delegates and proposal authors contributions are calculated

//2020 May, we for RQ2, we added pepdecsiondate in ppe details table to so that we can check the date the role of the a member as bdfl delegate when he made the decision 
...
public class SeparateStudyGroup2_May2020 extends JFrame
{	
	...
	static String pepDetailTable = "pepdetails_test"; //main one = 'pepdetails' //old";    //we changed this from pepdetails
	...
	static String reasonDataTable = "trainingdata_sept2020";
	
	static boolean updateReasonTable = true;
...
//							replySender = rs4.getString(4);
	...
	public static Date getPEPDecisionDate(Integer pep, Connection connection) {
		Date dateTime = null;	
		try{			
			String sql3 =   " SELECT date2 FROM pepstates_danieldata_datetimestamp_alltilldec2019 " + 
							" WHERE PEP = " + pep + 
							" AND (email LIKE '%accepted%' OR email LIKE '%rejected%' OR email LIKE '%final%') " + 
							" ORDER BY datetimestamp DESC " + 
							" LIMIT 1;";  //there will be many rows since there is no seprate table to store this information
			Statement stmt3 = connection.createStatement();			ResultSet rs3 = stmt3.executeQuery( sql3 );
			if (rs3.next()){    
				dateTime =  rs3.getDate(1);				
			}
			else {
				System.out.println("No decsion date foud for pep: " + pep + " so we gte date fro last state");
				//so we just get the date of last state
				String sql4 =  " SELECT date2 FROM pepstates_danieldata_datetimestamp_alltilldec2019 " + 
						" WHERE PEP = " + pep + 
						//" AND (email LIKE '%accepted%' OR email LIKE '%rejected%' OR email LIKE '%final%') " + 
						" ORDER BY datetimestamp DESC " + 
						" LIMIT 1;";  //there will be many rows since there is no seprate table to store this information
				Statement stmt4 = connection.createStatement();			ResultSet rs4 = stmt4.executeQuery( sql4 );
				if (rs4.next()){    
					dateTime =  rs4.getDate(1);				
				}
			}
		}
		catch (SQLException e)	{
	...
	private static void researchQuestionTwo(Integer pepType, Connection connection) {
		...
		try{	
			//member counts
			int bdflCounts=0,originalbdflCounts=0, pepeditor_counts=0, coredev_counts=0, dev_counts=0;
			int total_bdfl_messageCount =0;
			int totalNumberOfPEPs=0, totalNumberOfStandardPEPs=0,totalNumberOfInformationalPEPs=0, totalNumberOfProcessPEPs=0;
			
			//First we check if they are BdFL
		...
		//			"jesse noller","berker peksag","barry warsaw"};	
			pepEditors = rdcm.getallPEPEditorsWithDateAddedToPEPEditors(connection);	
			System.out.println("\n\nTotal pepeditors size " + pepEditors.size());				
			...
			System.out.println("Total Coredeveloeprs size " + coredevelopers.size());	
			String cd = "", pe= "";
			Date coredevdateaddedtolist = null, pepeditordateaddedtolist =null;
...
//					morePEPsAdded);
			...
			//----------------------LOOP FOR EACH PEP-------------------------------------------------
			String sql5 = "SELECT pep, authorCorrected,bdfl_delegateCorrected,type,created from "+pepDetailTable+";"; // WHERE pep = " + j + " and messageID BETWEEN 500000 AND 510000 order by dateTime;";  //there will be many rows since there is no seprate table to store this information			
			Statement stmt5 = connection.createStatement();	
			ResultSet rs5 = stmt5.executeQuery( sql5 );
			
			boolean authorFound=false, delegateFound=false;
			boolean pepHasDelegate = false; Date pepDecisionDate =null; //whether pep has delegate
			
			outerloop4:
			while (rs5.next()){   
				authorFound=false; delegateFound=false;  //reinitialise after each pep
				pepDecisionDate =null;	
				pepHasDelegate = false;
				...
				Date pepCreatedDate = rs5.getDate(5);
				
				if(bdflDelegate.length() > 0) {
					System.out.println("PEP " + pepNum +" has BDFL Delegate (" + bdflDelegate + ")" );
					pepDecisionDate = getPEPDecisionDate(pepNum, connection); //get the decision date on pep
					pepHasDelegate = true;
					originalbdflCounts++;
				}
				...
				totalNumberOfPEPs++;				
				if (type.toLowerCase().contains("process")){					totalNumberOfProcessPEPs++;				}	
				else if(type.toLowerCase().contains("standards track")){		totalNumberOfStandardPEPs++;			}	
				else if (type.toLowerCase().contains("informational")) {		totalNumberOfInformationalPEPs++;		}
				
				//BDFL-----------------------------------------------------------------------------
				//Check for each PEP, in the pepdetails table we check the BDFL delegate and pep author		
				if (author != null){
					author = author.toLowerCase();
					if (author.contains(bdfl)){
						authorFound=true;						count_BDFLAsAuthorsTotal++;														 // insertPEPAuthorRoleBDFLDelegate_forDebug(int pep, String peptype, String rolebeingcheckedfor, String author, String delegate, Connection connection)
						if (type.toLowerCase().contains("process")){				count_BDFLAsAuthorsP++;		insertPEPAuthorRoleBDFLDelegate_forDebug(pepNum, "process", "BDFL", "BDFL", "", connection);					}	
						else if(type.toLowerCase().contains("standards track")){	count_BDFLAsAuthorsS++;		insertPEPAuthorRoleBDFLDelegate_forDebug(pepNum, "standards track", "BDFL", "BDFL", "", connection);					}	
						else if (type.toLowerCase().contains("informational")) {	count_BDFLAsAuthorsI++;		insertPEPAuthorRoleBDFLDelegate_forDebug(pepNum, "informational", "BDFL", "BDFL", "", connection);					}
					}
				}
				...
				if (bdflDelegate != null){
					...
					if (bdflDelegate.contains(bdfl)){
						delegateFound = true; 					count_BDFLAsBDFLDelegateTotal++;
						if (type.toLowerCase().contains("process")){				count_BDFLAsBDFLDelegateP++;	insertPEPAuthorRoleBDFLDelegate_forDebug(pepNum, "process", "BDFL", "", "BDFL", connection);							}	
						else if(type.toLowerCase().contains("standards track")){	count_BDFLAsBDFLDelegateS++;	insertPEPAuthorRoleBDFLDelegate_forDebug(pepNum, "standards track", "BDFL", "", "BDFL", connection);							}	
						else if (type.toLowerCase().contains("informational")) {	count_BDFLAsBDFLDelegateI++;	insertPEPAuthorRoleBDFLDelegate_forDebug(pepNum, "informational", "BDFL", "", "BDFL", connection);							}	
					}
				}
				//PEP Editor----------------------------------------------------------------------------------
				/* old version
				for (pepeditor peped : pepEditors) {					
					pe = peped.getPEPeditor().trim().toLowerCase(); 
					if (author != null && !authorFound) {
						author = author.toLowerCase();
						if (author.contains(pe)){
							authorFound=true; 					count_pepEditorsAsAuthorsTotal++;														 // insertPEPAuthorRoleBDFLDelegate_forDebug(int pep, String peptype, String rolebeingcheckedfor, String author, String delegate, Connection connection)
					...
					if (bdflDelegate != null && !delegateFound) {
						bdflDelegate = bdflDelegate.toLowerCase();
						if (bdflDelegate.contains(pe)){
							delegateFound = true; 				count_pepEditorsAsBDFLDelegateTotal++;													       //insertPEPAuthorRoleBDFLDelegate_forDebug(int pep, String peptype, String rolebeingcheckedfor, String author, String delegate, Connection connection)
				...
				*/ 
				//new version
				for (pepeditor peped : pepEditors) {				
					pe = peped.getPEPeditor().trim().toLowerCase(); //System.out.println("\t"+pe + " as pep editor for pep "+ pepNum );	
					pepeditordateaddedtolist = (Date) peped.getDateaddedtopepeditorlist();
					if (author != null && !authorFound) {
						author = author.toLowerCase();
						if (author.contains(pe)  ||  author.equals(pe.trim())  ){
							// how we compare pep editors
							//if member added to pep editor list before pepcreation date, meaning at that data they were pep editors					
							if(pepeditordateaddedtolist.compareTo(pepCreatedDate) <= 0) {
							      ...
							      //   		+ "meaning pep was created when coredev was still a developer");							
									authorFound=true;	count_pepEditorsAsAuthorsTotal++;
									if (type.toLowerCase().contains("process")){				count_pepEditorsAsAuthorsP++;	insertPEPAuthorRoleBDFLDelegate_forDebug(pepNum, "process", "pepeditors", "pepeditors", "", connection);			}	
									else if(type.toLowerCase().contains("standards track")){	count_pepEditorsAsAuthorsS++;	insertPEPAuthorRoleBDFLDelegate_forDebug(pepNum, "standards track", "pepeditors", "pepeditors", "", connection);	}   
									else if (type.toLowerCase().contains("informational")) {	count_pepEditorsAsAuthorsI++;	insertPEPAuthorRoleBDFLDelegate_forDebug(pepNum, "informational", "pepeditors", "pepeditors", "", connection);	}

							//if member added to pep editorlist after pepcreation date, meaning at that data they were still core developer 
							//yes we asume that they would be coredev, and not normal contributors, because while we can further coorrelate with the date they were added to the core dev list
							// i dont think it is necessary
						    } else if(pepeditordateaddedtolist.compareTo(pepCreatedDate) > 0) {
//							         System.out.println("Date coredevdateaddedtolist occurs before Date pepCreatedDate");
						         authorFound=true;	count_coreDevelopersAsAuthorsTotal++;
						         if (type.toLowerCase().contains("process")){				count_coreDevelopersAsAuthorsP++;	insertPEPAuthorRoleBDFLDelegate_forDebug(pepNum, "process", "coredevelopers", "coredevelopers", "", connection);			}	
						         else if(type.toLowerCase().contains("standards track")){	count_coreDevelopersAsAuthorsS++;	insertPEPAuthorRoleBDFLDelegate_forDebug(pepNum, "standards track", "coredevelopers", "coredevelopers", "", connection);	}   
						         else if (type.toLowerCase().contains("informational")) {	count_coreDevelopersAsAuthorsI++;	insertPEPAuthorRoleBDFLDelegate_forDebug(pepNum, "informational", "coredevelopers", "coredevelopers", "", connection);	}
...
//					}
					//new version
					if (bdflDelegate != null && !delegateFound){
						//if coredevelopere matched
						if ( bdflDelegate.contains(pe.trim())  ||  bdflDelegate.equals(pe.trim())   )	{
							
							// how we compare codevelopers
							//if member added to pepeditorslist before pepDecisionDate date, meaning at that data they were pep editor
							if(pepeditordateaddedtolist.compareTo(pepDecisionDate) <= 0) {
						      //   System.out.println("For PEP "+ pepNum +" ignored, as Date coredev date added to coredevlist occurs after Date pepCreatedDate, "
						      //   		+ "meaning pep was created when coredev was still a developer");
								delegateFound = true; 	count_pepEditorsAsBDFLDelegateTotal++;
								if (type.toLowerCase().contains("process")){				count_pepEditorsAsBDFLDelegateP++;		insertPEPAuthorRoleBDFLDelegate_forDebug(pepNum, "process", "pepeditors", "", "pepeditors", connection);		}	
								else if(type.toLowerCase().contains("standards track")){	count_pepEditorsAsBDFLDelegateS++;		insertPEPAuthorRoleBDFLDelegate_forDebug(pepNum, "standards track", "pepeditors", "", "pepeditors", connection);   }
								else if (type.toLowerCase().contains("informational")) {	count_pepEditorsAsBDFLDelegateI++;		insertPEPAuthorRoleBDFLDelegate_forDebug(pepNum, "informational", "pepeditors", "", "pepeditors", connection);	 	}
	
								//may 2020 commented
								//if member added to core devlist after pepDecisionDate date, meaning at that data they were still a core developer
						    } else if(pepeditordateaddedtolist.compareTo(pepDecisionDate) > 0) {
						         System.out.println("Date coredev("+cd +") pepeditor addedtolist ("+pepeditordateaddedtolist+") occurs before BDFL pepdecisiondate ("+pepDecisionDate+") :: therefore allocated as core developer"); //System.out.println("Developer "+cd + " was bdfl delegate for pep "+ pepNum + " pepCreatedDate: " + pepCreatedDate);
								delegateFound = true; 	count_coreDevelopersAsBDFLDelegateTotal++;
								if (type.toLowerCase().contains("process")){				count_coreDevelopersAsBDFLDelegateP++;		insertPEPAuthorRoleBDFLDelegate_forDebug(pepNum, "process", "coredevelopers", "", "coredevelopers", connection);		}	
								else if(type.toLowerCase().contains("standards track")){	count_coreDevelopersAsBDFLDelegateS++;		insertPEPAuthorRoleBDFLDelegate_forDebug(pepNum, "standards track", "coredevelopers", "", "coredevelopers", connection);   }
								else if (type.toLowerCase().contains("informational")) {	count_coreDevelopersAsBDFLDelegateI++;		insertPEPAuthorRoleBDFLDelegate_forDebug(pepNum, "informational", "coredevelopers", "", "coredevelopers", connection);	 	}
				...
				for (coredev cdev : coredevelopers) {  //for (String cd : coredevelopers) {	
					...
					cd = cdev.getCoredev().trim().toLowerCase();  //System.out.println("core dev "+ cd);
					...
					if (author != null && !authorFound){
						//if coredevelopere matched
						if ( author.contains(cd.trim())  ||  author.equals(cd.trim())   )	{	
							...
							// how we compare core-developers
							//if member added to core devlist before pepcreation date, meaning at that data they were core developers
...
//						         System.out.println("Date coredevdateaddedtolist occurs before Date pepCreatedDate");
						         authorFound=true;	count_developersAsAuthorsTotal++;
						         if (type.toLowerCase().contains("process")){				count_developersAsAuthorsP++;	insertPEPAuthorRoleBDFLDelegate_forDebug(pepNum, "process", "contributor", "contributor", "", connection);			}	
						         else if(type.toLowerCase().contains("standards track")){	count_developersAsAuthorsS++;	insertPEPAuthorRoleBDFLDelegate_forDebug(pepNum, "standards track", "contributor", "contributor", "", connection);	}   
						         else if (type.toLowerCase().contains("informational")) {	count_developersAsAuthorsI++;	insertPEPAuthorRoleBDFLDelegate_forDebug(pepNum, "informational", "contributor", "contributor", "", connection);	}

						    } 
						}						
					}
					//CORE Developers as authors and BDFL delegates
					//we now check the bdfl delegates in core developers
					if (bdflDelegate != null && !delegateFound){
						//if coredevelopere matched
						if ( bdflDelegate.contains(cd.trim())  ||  bdflDelegate.equals(cd.trim())   )	{
							...
							// how we compare codevelopers
							//if member added to core devlist before pepDecisionDate date, meaning at that data they were still as contributor (only developer) 
							if(coredevdateaddedtolist.compareTo(pepDecisionDate) <= 0) {
						      //   System.out.println("For PEP "+ pepNum +" ignored, as Date coredev date added to coredevlist occurs after Date pepCreatedDate, "
						      //   		+ "meaning pep was created when coredev was still a developer");
								delegateFound = true; 	count_coreDevelopersAsBDFLDelegateTotal++;
								if (type.toLowerCase().contains("process")){				count_coreDevelopersAsBDFLDelegateP++;		insertPEPAuthorRoleBDFLDelegate_forDebug(pepNum, "process", "coredevelopers", "", "coredevelopers", connection);		}	
								...
								//may 2020 commented
								//if member added to core devlist after pepDecisionDate date, meaning at that data they were still as contributor (only developer) 
						    } else if(coredevdateaddedtolist.compareTo(pepDecisionDate) > 0) {
						         System.out.println("Date coredev("+cd +") coredevdateaddedtolist ("+coredevdateaddedtolist+") occurs before BDFL pepdecisiondate ("+pepDecisionDate+") :: therefore allocated as just developer"); //System.out.println("Developer "+cd + " was bdfl delegate for pep "+ pepNum + " pepCreatedDate: " + pepCreatedDate);
								delegateFound = true; 	count_developersAsBDFLDelegateTotal++;
								if (type.toLowerCase().contains("process")){				count_developersAsBDFLDelegateP++;		insertPEPAuthorRoleBDFLDelegate_forDebug(pepNum, "process", "contributor", "", "contributor", connection);		}	
								else if(type.toLowerCase().contains("standards track")){	count_developersAsBDFLDelegateS++;		insertPEPAuthorRoleBDFLDelegate_forDebug(pepNum, "standards track", "contributor", "", "contributor", connection);   }
								else if (type.toLowerCase().contains("informational")) {	count_developersAsBDFLDelegateI++;		insertPEPAuthorRoleBDFLDelegate_forDebug(pepNum, "informational", "contributor", "", "contributor", connection);	 	}
				...
				for (String d : allmembers) { 
					...
					//check if PEP authors
					if (author != null && !authorFound) {
						if (author.contains(d)     ||  author.equals(d.trim()) ){
							authorFound=true;	 count_developersAsAuthorsTotal++;															 // insertPEPAuthorRoleBDFLDelegate_forDebug(int pep, String peptype, String rolebeingcheckedfor, String author, String delegate, Connection connection)
							if (type.toLowerCase().contains("process")){				count_developersAsAuthorsP++;	insertPEPAuthorRoleBDFLDelegate_forDebug(pepNum, "process", "contributor", "contributor", "", connection);			}	
							else if(type.toLowerCase().contains("standards track")){	count_developersAsAuthorsS++;	insertPEPAuthorRoleBDFLDelegate_forDebug(pepNum, "standards track", "contributor", "contributor", "", connection);			}   
							else if (type.toLowerCase().contains("informational")) {	count_developersAsAuthorsI++;	insertPEPAuthorRoleBDFLDelegate_forDebug(pepNum, "informational", "contributor", "contributor", "", connection);			}
							//DONT NEED TO SHOW THIS
					...
					//check if BDFL delegates
					if (bdflDelegate != null && !delegateFound) {
						if (bdflDelegate.contains(d)  || bdflDelegate.equals(d.trim())   ){
							delegateFound = true; 	count_developersAsBDFLDelegateTotal++;															 // insertPEPAuthorRoleBDFLDelegate_forDebug(int pep, String peptype, String rolebeingcheckedfor, String author, String delegate, Connection connection)
							if (type.toLowerCase().contains("process")){				count_developersAsBDFLDelegateP++;		insertPEPAuthorRoleBDFLDelegate_forDebug(pepNum, "process", "contributor", "", "contributor", connection);		}	
							else if(type.toLowerCase().contains("standards track")){	count_developersAsBDFLDelegateS++;		insertPEPAuthorRoleBDFLDelegate_forDebug(pepNum, "standards track", "contributor", "", "contributor", connection);		}	
							else if (type.toLowerCase().contains("informational")) {	count_developersAsBDFLDelegateI++;		insertPEPAuthorRoleBDFLDelegate_forDebug(pepNum, "informational", "contributor", "", "contributor", connection);	    }	
							//DONT NEED TO SHOW THIS
							System.out.println("Developer "+d + " was bdfl delegate for pep "+ pepNum + " pepCreatedDate: " + pepCreatedDate);
						}
					}
				}
				
				if(!authorFound)
					System.out.println("\t Author notfound for pep "+ pepNum );
				if(pepHasDelegate && !delegateFound)
					System.out.println("\t PEP has delegate but BDFL delegate NOT MATCHED for PEP "+ pepNum );
			}			//END FOR EACH PEP		  
			
			System.out.println("\t total Number Of PEPs:" + totalNumberOfPEPs);
			System.out.println("\t total Number Of ProcessPEPs:" + totalNumberOfProcessPEPs);				
			System.out.println("\t total Number Of StandardPEPs:" + totalNumberOfStandardPEPs);				
			System.out.println("\t total Number Of InformationalPEPs:" +	totalNumberOfInformationalPEPs);		
			System.out.println("\t total Number Of BDFLs (one per pep):" + originalbdflCounts);
...
//						System.out.println("0 (dev) PEP Author Matched " + pepNum + " " +author + " d " + d);
	...
	public static void insertPEPAuthorRoleBDFLDelegate_forDebug(int pep, String peptype, String rolebeingcheckedfor, String authorRole, String bdflDelegateRole, Connection connection) {
		// create the java mysql update preparedstatement
	      ...
	      try {
	    	  ...
	    	  preparedStmt.setString(3, rolebeingcheckedfor);	    	  preparedStmt.setString(4, authorRole);			
	    	  preparedStmt.setString(5, bdflDelegateRole);
	      ...
	      if(updateReasonTable) {
		      String query2 = "update " + reasonDataTable +" SET authorRole=?,delegateRole=? WHERE PEP = ?; ";		      
		      try {
		    	  PreparedStatement preparedStmt = connection.prepareStatement(query2);		    	   	  
		    	  preparedStmt.setString(1, authorRole);	   	  preparedStmt.setString(2, bdflDelegateRole);
		    	  preparedStmt.setInt(3, pep); 	 
			      // execute the java preparedstatement
			      preparedStmt.executeUpdate();
		      }
			 catch (SQLException e) {
	      ...
	      if(updateRole) {
				updatePEPAuthorRole(pep,authorRole, bdflDelegateRole, connection);
				//System.out.println("\t0 here a");
			}
	      
	}
	
	public static void updatePEPAuthorRole(int pep, String authorRole, String bdflDelegateRole, Connection connection) {
		// create the java mysql update preparedstatement
	      String query = "update " + pepDetailTable +" set authorrole = ?, bdfldelegaterole =? where pep = ?";

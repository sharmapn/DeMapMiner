...
-- DROP TABLE pepthreadsbyperson2020;
-- lets first create this table easily
CREATE table pepthreadsbyperson2020 (id INT AUTO_INCREMENT PRIMARY KEY) AS
   -- distinct is just to make sure we dont get any duplicates -- dont worry it would not combine any of teh results 
	SELECT DISTINCT '1' AS threadlevel, pepnum2020, peptype2020, messageid, author, clusterbysenderfullname, emailmessageid, inreplyto, subject, authorsrole2020 -- ,  COUNT(messageid) AS cnt
	FROM allmessages
...
ALTER TABLE pepthreadsbyperson2020 ADD COLUMN threadid INT;
UPDATE pepthreadsbyperson2020 SET threadid = id;
...
-- DROP PROCEDURE createThreadInvolvementForEachPerson;
   ...
   CALL getThreads_Per_Message();
	...
	call getThreads_Per_Message_from_secondlevel(2); -- we strat with thread level = 2 and it would increment itself
	call getThreads_Per_Message_from_secondlevel(3); 
	call getThreads_Per_Message_from_secondlevel(4);
	call getThreads_Per_Message_from_secondlevel(5);
	call getThreads_Per_Message_from_secondlevel(6);
...
BEGIN
	...
	DECLARE v_inreplyto TEXT(15); DECLARE v_authorsrole2020 TEXT(20); 	DECLARE v_peptype2020 TEXT(20);  
	DECLARE curPEPList 
			CURSOR FOR 
			-- get all threads at top level, and then in the next procedure we get replies to these threads
			SELECT pepnum2020, messageid, author, clusterbysenderfullname, emailmessageid, subject, threadid, authorsrole2020, peptype2020 -- , inreplyto,  
				FROM pepthreadsbyperson2020; 		
...
getpepsstate: 
		LOOP
			FETCH curPEPList INTO v_pepnum2020,v_messageid, v_author, v_clusterbysenderfullname,  v_emailmessageid, v_subject, v_threadid, v_authorsrole2020, v_peptype2020;
			IF finished = 1 THEN 
				LEAVE getpepsstate;
			END IF;
			call createThreadInvolvementForEachPerson (v_pepnum2020,v_messageid, v_author, v_clusterbysenderfullname,  v_emailmessageid, 
																		v_subject, v_threadid, v_authorsrole2020, v_peptype2020, 2); -- pass 2 as threadlevel, we start at 1, which is the top level alrady explored
...
DELIMITER $$
CREATE PROCEDURE createThreadInvolvementForEachPerson (IN v_pepnum2020 INT(10), IN v_messageid INT(10), IN v_author TEXT(15), 
																		 IN v_clusterbysenderfullname TEXT(20), IN v_emailmessageid TEXT (20), IN v_subject TEXT (20), IN v_threadid INT(10),
																		 IN v_authorsrole2020 TEXT(20), IN v_peptype2020 TEXT(20), IN v_threadLevel INT(10))
...
getMessages: 
		LOOP
			FETCH curgetMessages INTO vv_pepnum2020,vv_messageid, vv_author, vv_clusterbysenderfullname,  vv_emailmessageid, vv_subject, vv_inreplyto, vv_authorsrole2020 ; 
			IF finished = 1 THEN 
				LEAVE getMessages;
			END IF;
			-- INSERT INTO SAME TABLE 
			INSERT INTO pepthreadsbyperson2020(threadlevel, pepnum2020,messageid, author, clusterbysenderfullname,  emailmessageid, subject, threadid, inreplyto, authorsrole2020, peptype2020 )
				VALUES (v_threadLevel, vv_pepnum2020,vv_messageid, vv_author, vv_clusterbysenderfullname, vv_emailmessageid, vv_subject, v_threadid, vv_inreplyto, vv_authorsrole2020, v_peptype2020 );			
...
-- DROP PROCEDURE createThreadInvolvementForEachPerson_from_secondlevel;
...
BEGIN
	...
	DECLARE curPEPList 
			...
			-- get all threads at top level, and then in the next procedure we get replies to these threads
			SELECT pepnum2020, messageid, author, clusterbysenderfullname, emailmessageid, subject, threadid, authorsrole2020, peptype2020 -- , inreplyto,  
				FROM pepthreadsbyperson2020 WHERE threadlevel = v_threadlevel; 		
				DECLARE CONTINUE HANDLER 
        		FOR NOT FOUND SET finished = 1;
OPEN curPEPList;
...
getpepsstate: 
		LOOP
			...
			IF finished = 1 THEN 
				LEAVE getpepsstate;
			END IF;
			call createThreadInvolvementForEachPerson_recursive_from_secondlevel (v_pepnum2020,v_messageid, v_author, v_clusterbysenderfullname,  v_emailmessageid, 
																		v_subject, v_threadid, v_authorsrole2020, v_peptype2020, v_threadlevel+1); -- add 1 to threadlevel, 
		END LOOP getpepsstate;
		CLOSE curPEPList;
...
DELIMITER $$
CREATE PROCEDURE createThreadInvolvementForEachPerson_from_secondlevel (IN v_pepnum2020 INT(10), IN v_messageid INT(10), IN v_author TEXT(15), 
																		 IN v_clusterbysenderfullname TEXT(20), IN v_emailmessageid TEXT (20), IN v_subject TEXT (20), IN v_threadid INT(10),
																		 IN v_authorsrole2020 TEXT(20), IN v_peptype2020 TEXT(20), IN v_threadLevel INT(10))
																		 -- IN v_inreplyto TEXT(20), IN v_authorsrole2020 TEXT(20)
BEGIN
